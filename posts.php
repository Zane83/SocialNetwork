...
<?php
		$id = $_SESSION['user_id'];
		$res = $mysqli->query("SELECT posts.id, posts.date_of_publication, posts.text, posts.id_user FROM posts INNER JOIN friendships ON (posts.id_user = friendships.id_sender OR posts.id_user = friendships.id_receiver) WHERE posts.id_user != $id AND (friendships.id_sender = $id OR friendships.id_receiver = $id) AND friendships.state = '1' ORDER BY posts.date_of_publication DESC, (SELECT level FROM user_interests WHERE id_1 = $id AND id_2 = posts.id_user) DESC ");
